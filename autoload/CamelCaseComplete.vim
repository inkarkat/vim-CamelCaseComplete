...
function! CamelCaseComplete#BuildRegexp( base, isWidenToEnsureMultiFragmentMatch )
	...
	if s:IsAlpha(l:anchor)
	    ...
	    if a:isWidenToEnsureMultiFragmentMatch && len(l:totalAlphabeticAnchors) == 1 && l:idx == len(l:anchors) - 1
...
"****D echomsg '####' '"'. l:anchor . '"'
    ...
    if a:isWidenToEnsureMultiFragmentMatch && len(l:alphabeticAnchorSequence) == 0
...
"****D echomsg '#### ...'
    ...
    return [l:strictRegexp, (l:relaxedRegexp ==# l:strictRegexp ? '' : l:relaxedRegexp)]
endfunction
function! s:WholeWordMatch( expr )
    return (empty(a:expr) ? '' : '\V\<' . a:expr . '\>')
endfunction
function! s:BuildRegexp( base )
    return map(CamelCaseComplete#BuildRegexp(a:base, 1), 's:WholeWordMatch(v:val)')
endfunction
